  ...
  static Future<List<League>> getLeagues() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        _log.fine('Decoded response type: ${data.runtimeType}');
        if (data is List && data.isNotEmpty && data[0] is Map) {
          try {
            _log.fine('First item keys: ${(data[0] as Map).keys.toList()}');
          } catch (_) {}
        }
        if (data is List) {
          ...
          _log.warning(
              'Unexpected data format: expected List, got ${data.runtimeType}');
        }
      } else {
        _log.severe(
            'Failed to fetch leagues. Status: ${response.statusCode}, Body: ${response.body}');
      }
      return [];
    } catch (e, stack) {
